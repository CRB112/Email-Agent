"""Inline editor for an email rule."""
...
from app.widgets.action_list_editor import ActionListEditor
...
class RuleEditor(ttk.Frame):
    def __init__(
        ...
        self._build_match_selector(allow_match_type_changes)
        ...
        ttk.Label(self, text="Actions:").grid(
            row=4, column=0, sticky="nw", padx=(0, 10), pady=3
        )
        self.action_editor = ActionListEditor(
            self,
            self.rule.get("modify", {}),
            allow_changes=allow_action_type_changes,
        )
        self.action_editor.grid(row=4, column=1, sticky="ew", pady=3)
    ...
    def _build_match_selector(self, allow_changes):
        ...
        if allow_changes:
            ...
            selector.grid(row=1, column=1, sticky="ew", pady=3)
            selector.bind("<<ComboboxSelected>>", self._change_match_type)
        else:
            ttk.Label(
                self,
                text=self.match_type_display.get() or "Not specified",
            ).grid(row=1, column=1, sticky="nw", pady=3)
    ...
    def save(self):
        ...
        try:
            ...
            actions = self.action_editor.values()
        except ValueError:
            self.error.config(
                text="Priority and numeric settings must be numbers."
            )
            return

        if not actions:
            self.error.config(text="Add at least one action to the rule.")
            return
